...
wget https://github.com/phoreproject/Phore/releases/download/v1.2.2/phore-1.2.2-x86_64-linux-gnu.tar.gz
tar -xvzf phore-1.2.2-x86_64-linux-gnu.tar.gz
cd phore-1.2.2/bin
sudo mv phore* /usr/local/bin/
cd
sudo rm -r phore-1.2.2
if [ $1 = "-u" ]; then
  phored -daemon
  phore-cli getinfo
  echo "アップデートは完了しました。"
